...
(function($) {
	$.fn.tipTip = function(options) {
		...
		if ($("#tiptip_holder").length <= 0) {
			var tiptip_holder = $('<div id="tiptip_holder" style="max-width:'
					+ opts.maxWidth + ';"></div>');
		...
		return this
				.each(function() {
					...
					if (org_title != "") {
						if (!opts.content) {
							org_elem.removeAttr(opts.attribute); // remove
																	// original
																	// Attribute
						}
						var timeout = false;
						...
						function active_tiptip() {
							...
							var right_compare = (w_compare + left) < parseInt($(
									window).scrollLeft());
							var left_compare = (tip_w + left) > parseInt($(
									window).width());
							...
							var top_compare = (top + org_height
									+ opts.edgeOffset + tip_h + 8) > parseInt($(
									window).height()
									+ $(window).scrollTop());
